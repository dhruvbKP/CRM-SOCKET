...
const registration = async (req, res) => {
    ...
    try {
        ...
        const { partnerid, name, secretkey, ipaddress, createdBy, modifiedBy } = req.body;
        const created = parseInt(createdBy);
        const modified = parseInt(modifiedBy);
        const partner_id = parseInt(partnerid);
        const insertData = await client.query(`
            SELECT public.insert_partner($1, $2, $3, $4, $5, $6)`,
            [partner_id, name, secretkey, ipaddress, created, modified]);
        if (insertData) {
